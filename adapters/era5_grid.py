...
def read_map(
    ...
    *,
    ...
    Срок, равный горизонту, принимается: `adapters.plan` отступает те же пять
    суток от срока прогона, и на текущем прогоне два отсчёта («от прогона» и
    «от сейчас») сходятся ровно в этой точке. Строгое сравнение здесь — не
    мелочь: с нестрогим план системы был бы неисполним в тот единственный
    момент, ради которого он и составлен.
    ...
    `target` в боевом пути передаёт загрузчик: сырой слой неизменяем и лежит
    по своему адресу (docs/PIPELINE.md §3), а умолчание кладёт файл в текущий
    каталог — это для одиночного вызова руками, не для конвейера.
    """
    at = _utc(moment)
